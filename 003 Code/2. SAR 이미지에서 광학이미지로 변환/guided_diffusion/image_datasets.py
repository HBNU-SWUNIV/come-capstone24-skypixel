...
def load_data(
    *,
    ...
    print("random_rotate: ", random_rotate)
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.random_flip and random.random() < 0.2:
            arr_sar = arr_sar[:, ::-1]
            arr_opt = arr_opt[:, ::-1]
        ...
        if self.random_rotate:
            ...
            if rotation_chance < 0.1:
                # 90도 회전
                arr_sar = np.rot90(arr_sar, 1)
                arr_opt = np.rot90(arr_opt, 1)
            elif rotation_chance < 0.2:
                # 180도 회전
                arr_sar = np.rot90(arr_sar, 2)
                arr_opt = np.rot90(arr_opt, 2)
            elif rotation_chance < 0.3:
                # 270도 회전
                arr_sar = np.rot90(arr_sar, 3)
                arr_opt = np.rot90(arr_opt, 3)
            # rotation_chance가 0.75 이상이면 회전하지 않음
