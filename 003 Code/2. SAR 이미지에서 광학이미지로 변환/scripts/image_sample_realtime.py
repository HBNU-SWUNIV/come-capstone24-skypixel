...
class pair_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img_sar, img_opt, os.path.basename(img_path_sar)

    def get_filename(self, index):
        return os.path.basename(self.all_img_sar[index])
...
def main():
    ...
    pair_datas = pair_Dataset('/workspace/seo/Dataset/scaling/val', transform_opt, transform_opt)
    data_loader = DataLoader(pair_datas, batch_size=args.batch_size, shuffle=False, num_workers=0)
    # num_samples를 입력 이미지의 개수로 조정
    num_samples = min(args.num_samples, len(pair_datas))
    print(f"Adjusted num_samples to: {num_samples}")
    count = 0
    ...
    while count * args.batch_size < num_samples:
        model_kwargs = {}
        all_images = []
        all_labels = []
        all_sar = []
        all_opt = []
        condition, gt, filenames = next(loader)
        ...
        for i in tqdm(range(l)):
            ...
            image_out.save(
                os.path.join(f'sample_results/{args.timestep_respacing}/gen_opt', f'{filename}'))
            ...
            image_opt.save(
                os.path.join(f'sample_results/{args.timestep_respacing}/gt_opt', f'{filename}'))
        count += 1
        if args.class_cond:
            gathered_labels = [
    ...
    arr_sar = np.concatenate(all_sar, axis=0)
    arr_sar = arr_sar[: num_samples]

    arr_opt = np.concatenate(all_opt, axis=0)
    arr_opt = arr_opt[: num_samples]
